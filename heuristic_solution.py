"""This proggram is a heuristic solution for KEP. Here we will try to find
combinations of 2 way and 3 way cycles which will create a good executable
solution."""

fin= open('200_pair.csv','r+')
fin.readline()
lines=fin.readlines()
...
for line in lines:
    mylist=line.split(',')
    Sr_no.append(str(mylist[0]))
    Recipient.append(str(mylist[1]))
    Recipient_Age.append(int(mylist[2]))
    Donor.append(str(mylist[3]))
    Donor_Age.append(int(mylist[4]))
    Blood_Group_Recipient.append(str(mylist[5]))
    Blood_Group_Donor.append(str(mylist[6]))
    HLA_A_Recipient.append(str(mylist[7]))
    HLA_A_Donor.append(str(mylist[8]))
    HLA_A_Recipient.append(str(mylist[9]))
    HLA_A_Donor.append(str(mylist[10]))
    HLA_B_Recipient.append(str(mylist[11]))
    HLA_B_Donor.append(str(mylist[12]))
    HLA_B_Recipient.append(str(mylist[13]))
    HLA_B_Donor.append(str(mylist[14]))
    HLA_B_Recipient.append(str(mylist[15]))
    HLA_B_Donor.append(str(mylist[16]))
    HLA_DR_Recipient.append(str(mylist[17]))
    HLA_DR_Donor.append(str(mylist[18]))
    HLA_DR_Recipient.append(str(mylist[19]))
    HLA_DR_Donor.append(str(mylist[20]))
    HLA_DR_Recipient.append(str(mylist[21]))
    HLA_DR_Donor.append(str(mylist[22]))



for i in range(len(Donor)):
    cycle3=[]
    if (Blood_Group_Donor[i]=='O' and Blood_Group_Donor[i]!='Matched') or (Blood_Group_Recipient[i]=='AB' and Blood_Group_Recipient[i]!='Matched') or (Blood_Group_Donor[i]==Blood_Group_Recipient[i] and Blood_Group_Recipient[i]!='Matched'):
        for j in range(len(Recipient)):
            for k in range(len(Recipient)):
                if Blood_Group_Donor[i]== Blood_Group_Recipient[j] and Blood_Group_Donor[j]== Blood_Group_Recipient[k] and Blood_Group_Donor[k]== Blood_Group_Recipient[i] and i!= j and j!= k and k!=i and Blood_Group_Donor[i]!='Matched' and Blood_Group_Recipient[i]!='Matched' and Blood_Group_Donor[j]!='Matched' and Blood_Group_Recipient[j]!='Matched' and Blood_Group_Donor[k]!='Matched' and Blood_Group_Recipient[k]!='Matched': 
                    #print ('We found a cycle')
                    cycle3.append(Donor[i])
                    cycle3.append(Recipient[j])
                    cycle3.append(Donor[j])
                    cycle3.append(Recipient[k])
                    cycle3.append(Donor[k])
                    cycle3.append(Recipient[i])
                    Blood_Group_Donor[i]='Matched'
                    Blood_Group_Donor[j]='Matched'
                    Blood_Group_Donor[k]='Matched'
                    Blood_Group_Recipient[i]='Matched'
                    Blood_Group_Recipient[j]='Matched'
                    Blood_Group_Recipient[k]='Matched'
                    break
                    

                    #print ("Cycle",Donor[i],Recipient[j],Donor[j],Recipient[k],Donor[k],Recipient[i])
    #print ('index',a,b,c)
    print ('3 Way cycle',cycle3)


#****************************************************************************#
#To find the swaps using Edmond's algorithm
...
#To compute number of swaps

No_edges=compatible_edges(Blood_Group_Recipient,Blood_Group_Donor)
Country_pairs=maxWeightMatching(No_edges, True)

count_1=0
for i in Country_pairs:
    if i!= -1:
        count_1=count_1+1

print ('Number of swaps',(count_1)/2)

# -*- coding: utf-8 -*-
"""
Created on Mon Apr 22 15:32:22 2019
...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 14 03:59:11 2019
...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 26 18:41:16 2018
...
def modified_graph(BG_rec, BG_don, n1, a1, b1,n2, a2, b2, n3, a3, b3, n4, a4, b4):
    ...
    for i in range(len(BG_don)):
        for j in range(len(BG_rec)):
            if (BG_don[i]=="A11" or BG_don[i]=="A22" or BG_don[i]=="A33" or BG_don[i]=="A44") and (BG_rec[j]=="A1" or BG_rec[j]=="A2" or BG_rec[j]=="A3" or BG_rec[j]=="A4"):
                Pairs.append((i,j,0))
                continue
...
#Finding value of the grand coalition for 20 rounds using theorem 3

#Simulation Plan
#1. Create list for the parameters to be recorded
#2. For 20 runs - generate pairs for each country between 10-25 in each run.
#3. First find matches for recipients without coalition, (Individual registries) and remove the pairs matches in the run
#4. With the same data set, run the program for grand coalition without using theorem 3 and find the individual shares.
#5. Find the shapley value for each country, solve the grand coalition according to theorem 3 and find the individual shares.
#6. Find the remaining credit for each country and add it to the shapley value of the next run.
#7. Store the difference between shapley value and alotted matches over 20 runs.
...
for x in range(5):
    ...
    Shapley_value1=[] # To Record the shapley value for each country
    Shapley_value2=[]
    Shapley_value3=[]
    Shapley_value4=[]
    
    Remaining_credit_country1=[] #To store the remaining credit for each country in SV case
    Remaining_credit_country2=[]
    Remaining_credit_country3=[]
    Remaining_credit_country4=[]
    ...
    Nucleous1=[] #to store the nucleous of each country at each stage
    Nucleous2=[]
    Nucleous3=[]
    Nucleous4=[]
    
    NU_Remaining_credit_country1=[] #To store the remaining credit for each country in Nucleous case 
    NU_Remaining_credit_country2=[]
    NU_Remaining_credit_country3=[]
    NU_Remaining_credit_country4=[]
    ...
    Number_after_match_country1=[] #Number of recipients remaining after each round in individual registry
    Number_after_match_country2=[]
    Number_after_match_country3=[]
    Number_after_match_country4=[]
    
    Total_number_country1=[] #Total Number of recipients considered for matching in each round 
    Total_number_country2=[]
    Total_number_country3=[]
    Total_number_country4=[]
    
    Individual_sol_country1=[] #To record the optimal solution for each country if countries are match individually
    Individual_sol_country2=[]
    Individual_sol_country3=[]
    Individual_sol_country4=[]
    
    Random_sol_country1=[] # To record the solution for each country if assigned randomly in Grand Coalition
    Random_sol_country2=[]
    Random_sol_country3=[]
    Random_sol_country4=[]
    ...
    Ind_dropout_country1=0
    Ind_dropout_country2=0
    Ind_dropout_country3=0
    Ind_dropout_country4=0
    
    Ran_dropout_country1=0
    Ran_dropout_country2=0
    Ran_dropout_country3=0
    Ran_dropout_country4=0
    ...
    Ind_Waiting_time_country1=0
    Ind_Waiting_time_country2=0
    Ind_Waiting_time_country3=0
    Ind_Waiting_time_country4=0
    ...
    for z in range(2):
        print ('Round',z)
        random1=int(random.uniform(10,20)) #Number of pairs needed to be generated for country 1
        random2=int(random.uniform(10,20))
        random3=int(random.uniform(10,20))
        random4=int(random.uniform(10,20))
        ...
        Total_number_country1.append(count1)
        Total_number_country2.append(count2)
        Total_number_country3.append(count3)
        Total_number_country4.append(count4)
        ...
        Individual_sol_country1.append(Country1_count)
        ...
        Individual_sol_country2.append(Country2_count)
        ...
        Individual_sol_country3.append(Country3_count)
        ...
        Individual_sol_country4.append(Country4_count)
        ...
        for i in range(len(Recipient_BG_country1_total)):
            ...
            if Recipient_BG_country1_total[i]!='Matched' and k1>=p:
                ...
                Ind_Waiting_time_country1=Ind_Waiting_time_country1 + 1
            elif Recipient_BG_country1_total[i]!='Matched' and k1 <=p:
                Ind_dropout_country1=Ind_dropout_country1+1    
        ...
        for i in range(len(Recipient_BG_country2_total)):
            ...
            if Recipient_BG_country2_total[i]!='Matched' and k2>=p:
                ...
                Ind_Waiting_time_country2=Ind_Waiting_time_country2 + 1
            elif Recipient_BG_country2_total[i]!='Matched' and k2 <=p:
                Ind_dropout_country2=Ind_dropout_country2+1
        ...
        for i in range(len(Recipient_BG_country3_total)):
            ...
            if Recipient_BG_country3_total[i]!='Matched' and k3>=p:
                ...
                Ind_Waiting_time_country3=Ind_Waiting_time_country3 + 1
            elif Recipient_BG_country3_total[i]!='Matched' and k3 <=p:
                Ind_dropout_country3=Ind_dropout_country3+1
        ...
        Number_after_match_country1.append(len(Recipient_BG_country1_total))
        Number_after_match_country2.append(len(Recipient_BG_country2_total))
        Number_after_match_country3.append(len(Recipient_BG_country3_total))
        Number_after_match_country4.append(len(Recipient_BG_country4_total))
        #end of program for Individual registry analysis
        ...
        Random_sol_country1.append(Country1_count)
        Random_sol_country2.append(Country2_count)
        Random_sol_country3.append(Country3_count)
        Random_sol_country4.append(Country4_count)
        ...
        for i in range(len(GR_BG_Rec_country1)):
            ...
            if GR_BG_Rec_country1[i]!='Matched' and g1>=p:
                ...
                Ran_dropout_country1=Ran_dropout_country1+1
        ...
        for i in range(len(GR_BG_Rec_country2)):
            ...
            if GR_BG_Rec_country2[i]!='Matched' and g2>=p:
                ...
                Ran_dropout_country2=Ran_dropout_country2+1
        ...
        for i in range(len(GR_BG_Rec_country3)):
            ...
            if GR_BG_Rec_country3[i]!='Matched' and g3>=p:
                ...
                Ran_dropout_country3=Ran_dropout_country3+1
        ...
        for i in range(len(GR_BG_Rec_country4)):
            ...
            if GR_BG_Rec_country4[i]!='Matched' and g4>=p:
                ...
                Ran_dropout_country4=Ran_dropout_country4+1
        ...
        #end of program for Random Global allocation
    
        #Program for finding fair solution in Grand coalition using shapley value
        ...
        Shapley_value1.append(shapley_values[0])
        Shapley_value2.append(shapley_values[1])
        Shapley_value3.append(shapley_values[2])
        Shapley_value4.append(shapley_values[3])
        ...
        Country3_count=0
        Country4_count=0
        #print ('SV_Country_matched_index before',SV_Country_matched_index)
        #print ('SV_BG_Rec_country1 before', SV_BG_Rec_country1)
...
# =============================================================================
#         print ('SV_Country_matched_index after',SV_Country_matched_index)
#         print ('Total number of nodes',len(SV_Country_matched_index),'n1+n2+n3+n4',n1+n2+n3+n4)
#         print ('SV_BG_Rec_country1 after', SV_BG_Rec_country1)
#     
# =============================================================================
        ...
        Remaining_credit_country1.append(round(RC_C1,3))
        Remaining_credit_country2.append(round(RC_C2,3))
        Remaining_credit_country3.append(round(RC_C3,3))
        Remaining_credit_country4.append(round(RC_C4,3))
        ...
        Nucleous1.append(NU[0])
        Nucleous2.append(NU[1])
        Nucleous3.append(NU[2])
        Nucleous4.append(NU[3])
        ...
        NU_Remaining_credit_country1.append(round(NU_RC_C1,3))
        NU_Remaining_credit_country2.append(round(NU_RC_C2,3))
        NU_Remaining_credit_country3.append(round(NU_RC_C3,3))
        NU_Remaining_credit_country4.append(round(NU_RC_C4,3))
    ...
    ##    country4=0
    ##    for elem in P:
    ##        lines = "    Index " + str(indx) + " is matched to "  + str(elem) + " \n"
    ##        #print (lines)
    ##        if indx <=n1+n2+n3+n4-1:
    ##            if elem!=-1 and elem <=n1-1:
    ##                country1=country1+1
    ##            if elem!=-1 and elem <=n1+n2-1 and elem >=n1:
    ##                country2=country2+1
    ##            if elem!=-1 and elem <=n1+n2+n3-1 and elem >=n1+n2:
    ##                country3=country3+1
    ##            if elem!=-1 and elem <=n1+n2+n3+n4-1 and elem >=n1+n2+n3:
    ##                country4=country4+1
    ##            if indx ==n1+n2+n3+n4-1:
    ##                break
    ##        indx=indx+1
...
# =============================================================================
#     print ('Number of recipients generated for country 1 at each stage', Number_recipient_country1)
#     print ('Number of recipients generated for country 2 at each stage', Number_recipient_country2)
#     print ('Number of recipients generated for country 3 at each stage', Number_recipient_country3)
#     print ('Number of recipients generated for country 4 at each stage', Number_recipient_country4)
#     
#     print ('Number of recipients considered for matching for country 1 at each stage', Total_number_country1)
#     print ('Number of recipients considered for matching for country 2 at each stage', Total_number_country2)
#     print ('Number of recipients considered for matching for country 3 at each stage', Total_number_country3)
#     print ('Number of recipients considered for matching for country 4 at each stage', Total_number_country4)
#     
#     print ('Number of recipients matched for country 1 at each stage', Individual_sol_country1,'Total',sum(Individual_sol_country1))
#     print ('Number of recipients matched for country 2 at each stage', Individual_sol_country2,'Total',sum(Individual_sol_country2))
#     print ('Number of recipients matched for country 3 at each stage', Individual_sol_country3,'Total',sum(Individual_sol_country3))
#     print ('Number of recipients matched for country 4 at each stage', Individual_sol_country4,'Total',sum(Individual_sol_country4))
#     
# =============================================================================
    I=sum(Individual_sol_country1)+sum(Individual_sol_country2)+sum(Individual_sol_country3)+sum(Individual_sol_country4)
    
# =============================================================================
#     print ('Total number of recipients matched over 20 rounds in individual registries',I)
#     
#     print ('Number of recipients matched for country 1 in Global coalition at each stage', Random_sol_country1,'Total',sum(Random_sol_country1))
#     print ('Number of recipients matched for country 2 in Global coalition at each stage', Random_sol_country2,'Total',sum(Random_sol_country2))
#     print ('Number of recipients matched for country 3 in Global coalition at each stage', Random_sol_country3,'Total',sum(Random_sol_country3))
#     print ('Number of recipients matched for country 4 in Global coalition at each stage', Random_sol_country4,'Total',sum(Random_sol_country4))
#     
# =============================================================================
    R=sum(Random_sol_country1)+sum(Random_sol_country2)+sum(Random_sol_country3)+sum(Random_sol_country4)
    
# =============================================================================
#     print ('Total number of recipients matched over 20 rounds in Global coalition',R)
#     
#     print ('Number of recipients matched using credit system for country 1 in Global coalition at each stage', SV_sol_country1,'Total',sum(SV_sol_country1))
#     print ('Number of recipients matched using credit system for country 2 in Global coalition at each stage', SV_sol_country2,'Total',sum(SV_sol_country2))
#     print ('Number of recipients matched using credit system for country 3 in Global coalition at each stage', SV_sol_country3,'Total',sum(SV_sol_country3))
#     print ('Number of recipients matched using credit system for country 4 in Global coalition at each stage', SV_sol_country4,'Total',sum(SV_sol_country4))
#     
# =============================================================================
    S=sum(SV_sol_country1)+sum(SV_sol_country2)+sum(SV_sol_country3)+sum(SV_sol_country4)
    
# =============================================================================
#     print ('Total number of recipients matched over 20 rounds in Global coalition using credit system',S)
#     
#     print ('Number of recipients matched using Nucleous as credit for country 1 in Global coalition at each stage', NU_sol_country1,'Total',sum(NU_sol_country1))
#     print ('Number of recipients matched using Nucleous as credit for country 2 in Global coalition at each stage', NU_sol_country2,'Total',sum(NU_sol_country2))
#     print ('Number of recipients matched using Nucleous as credit for country 3 in Global coalition at each stage', NU_sol_country3,'Total',sum(NU_sol_country3))
#     print ('Number of recipients matched using Nucleous as credit for country 4 in Global coalition at each stage', NU_sol_country4,'Total',sum(NU_sol_country4))
#     
# =============================================================================
    N=sum(NU_sol_country1)+sum(NU_sol_country2)+sum(NU_sol_country3)+sum(NU_sol_country4)
# =============================================================================
#     print ('Total number of recipients matched over 20 rounds in Global coalition using Nucleous as credit',N)
#     
#     print ('Shapley Value Country 1',Shapley_value1, 'Average', np.mean(Shapley_value1))
#     print ('Shapley Value Country 2',Shapley_value2, 'Average', np.mean(Shapley_value2))
#     print ('Shapley Value Country 3',Shapley_value3, 'Average', np.mean(Shapley_value3))
#     print ('Shapley Value Country 4',Shapley_value4, 'Average', np.mean(Shapley_value4))
#     
#     print ('Nucleous Country 1',Nucleous1, 'Average', np.mean(Nucleous1))
#     print ('Nucleous Country 2',Nucleous2, 'Average', np.mean(Nucleous2))
#     print ('Nucleous Country 3',Nucleous3, 'Average', np.mean(Nucleous3))
#     print ('Nucleous Country 4',Nucleous4, 'Average', np.mean(Nucleous4))
#     
#     print ('Total number of Dropouts country 1 in Individual allocation', Ind_dropout_country1)
#     print ('Total number of Dropouts country 2 in Individual allocation', Ind_dropout_country2)
#     print ('Total number of Dropouts country 3 in Individual allocation', Ind_dropout_country3)
#     print ('Total number of Dropouts country 4 in Individual allocation', Ind_dropout_country4)
#     
#     print ('Total number of Dropouts country 1 in Global random allocation', Ran_dropout_country1)
#     print ('Total number of Dropouts country 2 in Global random allocation', Ran_dropout_country2)
#     print ('Total number of Dropouts country 3 in Global random allocation', Ran_dropout_country3)
#     print ('Total number of Dropouts country 4 in Global random allocation', Ran_dropout_country4)
#     
#     print ('Total number of Dropouts country 1 in credit allocation through Shapley value', SV_dropout_country1)
#     print ('Total number of Dropouts country 2 in credit allocation through Shapley value', SV_dropout_country2)
#     print ('Total number of Dropouts country 3 in credit allocation through Shapley value', SV_dropout_country3)
#     print ('Total number of Dropouts country 4 in credit allocation through Shapley value', SV_dropout_country4)
#     
#     print ('Total number of Dropouts country 1 in credit allocation through Nucleous', NU_dropout_country1)
#     print ('Total number of Dropouts country 2 in credit allocation through Nucleous', NU_dropout_country2)
#     print ('Total number of Dropouts country 3 in credit allocation through Nucleous', NU_dropout_country3)
#     print ('Total number of Dropouts country 4 in credit allocation through Nucleous', NU_dropout_country4)
#     
#     
# =============================================================================
    #print ('Remaining Credit for country 1 in each round',Remaining_credit_country1)
    #print ('Remaining Credit for country 2 in each round',Remaining_credit_country2)
    #print ('Remaining Credit for country 3 in each round',Remaining_credit_country3)
    #print ('Remaining Credit for country 4 in each round',Remaining_credit_country4)
    
    
    #Graph Analysis
    y1=[sum(Individual_sol_country1),sum(Individual_sol_country2),sum(Individual_sol_country3),sum(Individual_sol_country4),I]
    y2=[sum(Random_sol_country1),sum(Random_sol_country2),sum(Random_sol_country3),sum(Random_sol_country4),R]
    y3=[sum(SV_sol_country1),sum(SV_sol_country2),sum(SV_sol_country3),sum(SV_sol_country4),S]
    y4=[sum(NU_sol_country1),sum(NU_sol_country2),sum(NU_sol_country3),sum(NU_sol_country4),N]
    
    
    #Individual vs Global Random
    xpos=np.arange(5)
    plt.xticks(xpos,('Country1','Country2','Country3','Country4','Total'))
    plt.bar(xpos-0.15, y1, color = 'grey', width = 0.25, label='Individual allocation')
    plt.bar(xpos+0.15, y2, color = 'red', width = 0.25, label='Global random allocation')
    plt.xlabel('Countries and total')
    plt.ylabel('Total number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation and global random allocation')
    plt.legend()
    plt.show()
    
    #Individual vs Credit based global allocation
    xpos=np.arange(5)
    plt.xticks(xpos,('Country1','Country2','Country3','Country4','Total'))
    plt.bar(xpos-0.15, y1, color = 'grey', width = 0.25, label='Individual allocation')
    plt.bar(xpos+0.15, y3, color= 'blue',width=0.25, label='Global credit based allocation')
    plt.xlabel('Countries and total')
    plt.ylabel('Total number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation and credit based global allocation')
    plt.legend()
    plt.show()
    
    #Individual vs Credit based global allocation using Nucleous as target solution
    xpos=np.arange(5)
    plt.xticks(xpos,('Country1','Country2','Country3','Country4','Total'))
    plt.bar(xpos-0.15, y1, color = 'grey', width = 0.25, label='Individual allocation')
    plt.bar(xpos+0.15, y4, color= 'green',width=0.25, label='Credit based allocation using Nucleous')
    plt.xlabel('Countries and total')
    plt.ylabel('Total number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation and credit based global allocation using Nucleous')
    plt.legend()
    plt.show()
    
    #Individual vs Global vs Credit based
    xpos=np.arange(5)
    plt.xticks(xpos,('Country1','Country2','Country3','Country4','Total'))
    ...
    plt.xlabel('Countries and total')
    plt.ylabel('Total number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation vs Global random allocation vs credit based global allocation')
    plt.legend()
    plt.show()
    
    #Country 1
    xpos=np.arange(1,len(Individual_sol_country1)+1)
    y1=Individual_sol_country1
    y2=Random_sol_country1
    y3=SV_sol_country1
    y4=NU_sol_country1
    
    plt.plot(xpos, y1, color = 'grey', label='Individual allocation')
    plt.plot(xpos, y2, color = 'red', label='Global random allocation')
    plt.plot(xpos, y3, color= 'blue', label='Shapley based allocation')
    plt.plot(xpos, y4, color= 'green', label='Nucleous based allocation')
    plt.xlabel('Number of rounds')
    plt.ylabel('Number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation, global random allocation and credit based global allocation for country 1')
    plt.legend()
    plt.show()
    
    #Country 2
    xpos=np.arange(1,len(Individual_sol_country2)+1)
    y1=Individual_sol_country2
    y2=Random_sol_country2
    y3=SV_sol_country2
    y4=NU_sol_country2
    ...
    plt.xlabel('Number of rounds')
    plt.ylabel('Number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation, global random allocation and credit based global allocation for country 2')
    plt.legend()
    plt.show()
    
    #Country 3
    xpos=np.arange(1,len(Individual_sol_country3)+1)
    y1=Individual_sol_country3
    y2=Random_sol_country3
    y3=SV_sol_country3
    y4=NU_sol_country3
    ...
    plt.xlabel('Number of rounds')
    plt.ylabel('Number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation, global random allocation and credit based global allocation for country 3')
    plt.legend()
    plt.show()
    
    #Country 4
    xpos=np.arange(1,len(Individual_sol_country4)+1)
    y1=Individual_sol_country4
    y2=Random_sol_country4
    y3=SV_sol_country4
    y4=NU_sol_country4
    ...
    plt.xlabel('Number of rounds')
    plt.ylabel('Number of recipients matched over 20 runs')
    plt.title('Comparison between individual country allocation, global random allocation and credit based global allocation for country 4')
    plt.legend()
    plt.show()
    ...
    #Graph Analysis
    y1=[sum(Individual_sol_country1),sum(Individual_sol_country2),sum(Individual_sol_country3),sum(Individual_sol_country4)]
    y2=[sum(Random_sol_country1),sum(Random_sol_country2),sum(Random_sol_country3),sum(Random_sol_country4)]
    y3=[sum(SV_sol_country1),sum(SV_sol_country2),sum(SV_sol_country3),sum(SV_sol_country4)]
    y4=[sum(NU_sol_country1),sum(NU_sol_country2),sum(NU_sol_country3),sum(NU_sol_country4)]
    
    
    Diff_individual_random=[]
    Diff_individual_credit=[]
    Diff_individual_nucleous=[]
    Total_number_of_recipients=[sum(Number_recipient_country1),sum(Number_recipient_country2),sum(Number_recipient_country3),sum(Number_recipient_country4)]
    for i in range(len(y1)):
        Diff_individual_random.append((y2[i]-y1[i])/Total_number_of_recipients[i])
        Diff_individual_credit.append((y3[i]-y1[i])/Total_number_of_recipients[i])
        Diff_individual_nucleous.append((y4[i]-y1[i])/Total_number_of_recipients[i])
    
    xpos=np.arange(4)
    plt.xticks(xpos,('Country1','Country2','Country3','Country4'))
    plt.bar(xpos-0.15, Diff_individual_credit, color = 'Red', width = 0.25, label='Global coailition with Shapley value')
    plt.bar(xpos+0.15, Diff_individual_nucleous, color = 'Green', width = 0.25, label='Global coailition with Nucleous')
    plt.xlabel('Countries')
    plt.ylabel('Relative benefit over 20 runs')
    plt.title('Comparison of relative benefits of credit based allocations for different size countries')
    plt.legend()
    plt.show()
    plt.savefig('Different_size_comparison')
    plt.close()
    
    z1=[Ind_dropout_country1,Ind_dropout_country2,Ind_dropout_country3,Ind_dropout_country4,Ind_dropout_country1+Ind_dropout_country2+Ind_dropout_country3+Ind_dropout_country4]
    z2=[Ran_dropout_country1,Ran_dropout_country2,Ran_dropout_country3,Ran_dropout_country4,Ran_dropout_country1+Ran_dropout_country2+Ran_dropout_country3+Ran_dropout_country4]
    z3=[SV_dropout_country1,SV_dropout_country2,SV_dropout_country3,SV_dropout_country4,SV_dropout_country1+SV_dropout_country2+SV_dropout_country3+SV_dropout_country4]
    z4=[NU_dropout_country1,NU_dropout_country2,NU_dropout_country3,NU_dropout_country4,NU_dropout_country1+NU_dropout_country2+NU_dropout_country3+NU_dropout_country4]
    
    xpos=np.arange(5)
    plt.xticks(xpos,('Country1','Country2','Country3','Country4','Total'))
    plt.bar(xpos-0.30, z1, color = 'grey', width = 0.15, label='Individual allocation')
    plt.bar(xpos-0.10, z2, color = 'red', width = 0.15, label='Global random allocation')
    plt.bar(xpos+0.10, z3, color= 'blue',width=0.15, label='Credit based allocation using Shapley Value')
    plt.bar(xpos+0.30, z4, color= 'green',width=0.15, label='Credit based allocation using Nucleous')
    plt.xlabel('Countries and total')
    plt.ylabel('Total number of recipients dropedout over 20 runs')
    plt.title('Dropout comparion for different allocations')
    plt.legend()
    plt.show()
    
# =============================================================================
#     print ('Total waiting time in Individual allocation for country 1', Ind_Waiting_time_country1)
#     print ('Total waiting time in Individual allocation for country 2', Ind_Waiting_time_country2)
#     print ('Total waiting time in Individual allocation for country 3', Ind_Waiting_time_country3)
#     print ('Total waiting time in Individual allocation for country 4', Ind_Waiting_time_country4)
#     
#     print ('Total waiting time in Global Random allocation for country 1', GR_Waiting_time_country1)
#     print ('Total waiting time in Global Random allocation for country 2', GR_Waiting_time_country2)
#     print ('Total waiting time in Global Random allocation for country 3', GR_Waiting_time_country3)
#     print ('Total waiting time in Global Random allocation for country 4', GR_Waiting_time_country4)
#     
#     print ('Total waiting time in Shapley Value allocation for country 1', SV_Waiting_time_country1)
#     print ('Total waiting time in Shapley Value allocation for country 2', SV_Waiting_time_country2)
#     print ('Total waiting time in Shapley Value allocation for country 3', SV_Waiting_time_country3)
#     print ('Total waiting time in Shapley Value allocation for country 4', SV_Waiting_time_country4)
#     
#     print ('Total waiting time in Nucleaous allocation for country 1', NU_Waiting_time_country1)
#     print ('Total waiting time in Nucleaous allocation for country 2', NU_Waiting_time_country2)
#     print ('Total waiting time in Nucleaous allocation for country 3', NU_Waiting_time_country3)
#     print ('Total waiting time in Nucleaous allocation for country 4', NU_Waiting_time_country4)
#     
# =============================================================================
    Ind_average_waiting_time=3*(Ind_Waiting_time_country1+Ind_Waiting_time_country2+Ind_Waiting_time_country3+Ind_Waiting_time_country4)/sum(Total_number_of_recipients)
    GR_average_waiting_time=3*(GR_Waiting_time_country1+GR_Waiting_time_country2+GR_Waiting_time_country3+GR_Waiting_time_country4)/sum(Total_number_of_recipients)
    SV_average_waiting_time=3*(SV_Waiting_time_country1+SV_Waiting_time_country2+SV_Waiting_time_country3+SV_Waiting_time_country4)/sum(Total_number_of_recipients)
    NU_average_waiting_time=3*(NU_Waiting_time_country1+NU_Waiting_time_country2+NU_Waiting_time_country3+NU_Waiting_time_country4)/sum(Total_number_of_recipients)
    
# =============================================================================
#     print ('Total waiting time for all 4 countries in Individual allocation',Ind_average_waiting_time)
#     print ('Total waiting time for all 4 countries in Global Random allocation',GR_average_waiting_time)
#     print ('Total waiting time for all 4 countries in Shapley Value allocation',SV_average_waiting_time)
#     print ('Total waiting time for all 4 countries in Nucleaous allocation',NU_average_waiting_time)
#     
# =============================================================================
    
    Waiting_time=[Ind_average_waiting_time,GR_average_waiting_time,SV_average_waiting_time,NU_average_waiting_time]
    
    xpos=np.arange(4)
    plt.xticks(xpos,('Individual allocation','Global Random allocation','Shapley value based alloation','Nucleous based allocation'))
    plt.bar(xpos-0.15, Waiting_time, color = 'Grey', width = 0.25, label='Waiting time for all cases')
    plt.xlabel('Different allocation schemes')
    plt.ylabel('Average waiting time in months')
    plt.title('Comparison of average waiting times of different proposed allocations')
    plt.legend()
    plt.show()
    plt.savefig('Waiting time')
    plt.close()

    Ind_C1_total.append(sum(Individual_sol_country1))
    Ind_C2_total.append(sum(Individual_sol_country2))
    Ind_C3_total.append(sum(Individual_sol_country3))
    Ind_C4_total.append(sum(Individual_sol_country4))

    GR_C1_total.append(sum(Random_sol_country1))
    GR_C2_total.append(sum(Random_sol_country2))
    GR_C3_total.append(sum(Random_sol_country3))
    GR_C4_total.append(sum(Random_sol_country4))
    
    SV_C1_total.append(sum(SV_sol_country1))
    SV_C2_total.append(sum(SV_sol_country2))
    SV_C3_total.append(sum(SV_sol_country3))
    SV_C4_total.append(sum(SV_sol_country4))
    
    NU_C1_total.append(sum(NU_sol_country1))
    NU_C2_total.append(sum(NU_sol_country2))
    NU_C3_total.append(sum(NU_sol_country3))
    NU_C4_total.append(sum(NU_sol_country4))



#Graph analysis in multiple rounds
    
#Graph Analysis

I=np.mean(Ind_C1_total)+np.mean(Ind_C2_total)+np.mean(Ind_C3_total)+np.mean(Ind_C4_total)
R=np.mean(GR_C1_total)+np.mean(GR_C2_total)+np.mean(GR_C3_total)+np.mean(GR_C4_total)
S=np.mean(SV_C1_total)+np.mean(SV_C2_total)+np.mean(SV_C3_total)+np.mean(SV_C4_total)
N=np.mean(NU_C1_total)+np.mean(NU_C2_total)+np.mean(NU_C3_total)+np.mean(NU_C4_total)

y1=[np.mean(Ind_C1_total),np.mean(Ind_C2_total),np.mean(Ind_C3_total),np.mean(Ind_C4_total),I]
y2=[np.mean(GR_C1_total),np.mean(GR_C2_total),np.mean(GR_C3_total),np.mean(GR_C4_total),R]
y3=[np.mean(SV_C1_total),np.mean(SV_C2_total),np.mean(SV_C3_total),np.mean(SV_C4_total),S]
y4=[np.mean(NU_C1_total),np.mean(NU_C2_total),np.mean(NU_C3_total),np.mean(NU_C4_total),N]
...
plt.bar(xpos+0.30, y4, color= 'green',width=0.15, label='Credit based allocation using Nucleous')
plt.xlabel('Countries and total')
plt.ylabel('Avg number recipients matched over 20 runs and 30 replications')
plt.title('Comparison between individual country allocation vs Global random allocation vs credit based global allocation')
plt.legend()
plt.show()
